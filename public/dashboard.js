...
document
  ...
  .addEventListener("click", async () => {
    ...
    const response = await fetch(
      `http://localhost:8001/flights/search?origin=${countryFrom}&destination=${countryTo}`
    );
    ...
    flights.forEach((flight) => {
      ...
      li.innerHTML = `${flight.airline} - ${flight.origin} to ${flight.destination} on ${flight.date} - $${flight.price}
...
function selectFlight(flightId) {
  ...
  document.getElementById(
    "flight-details"
  ).innerHTML = `${flight.airline} - ${flight.from} to ${flight.to} on ${flight.date}`;
...
function displayBookings() {
  ...
  bookings.forEach((booking, index) => {
    let li = document.createElement("li");
    li.innerHTML = `Booking ID: ${index + 1} - ${booking.airline} - ${
      booking.from
    } to ${booking.to} on ${booking.date}
